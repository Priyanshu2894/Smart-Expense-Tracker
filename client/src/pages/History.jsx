...
function History() {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        // Connect to generalized transaction endpoint (routed internally as /api/expenses)
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this transaction permanently from the ledger?")) return;
    try {
      const token = localStorage.getItem('token');
      ...
      if (response.ok) {
        ...
        alert("Failed to delete transaction. It may have already been removed.");
      }
    } catch (err) {
      console.error(err);
    }
